...
export default function Home() {
  ...
  return (
    <main ref={containerRef} className="relative w-full h-screen overflow-y-auto overflow-x-hidden md:snap-y md:snap-mandatory no-scrollbar bg-[#0a0807] text-salvore-cream selection:bg-salvore-caramel selection:text-black">
      ...
      {/* PANEL 3: MENU GRID - TIGHT SPACING FIX */}
                ...
                {menuData[activeCategory].map((item) => (
                  <div key={item.name} className="glass-panel-luxury rounded-2xl border border-white/5 overflow-hidden flex flex-col md:flex-row h-auto md:h-44 group hover:border-salvore-caramel/40 transition-all">
                    <div className="w-full md:w-1/3 h-40 md:h-full relative overflow-hidden bg-black">
                      <img src={item.image} className="w-full h-full object-cover grayscale-0 md:grayscale md:group-hover:grayscale-0 transition-all duration-700" alt={item.name} />
                    </div>
      ...
      {/* PANEL 4: GALLERY - NO VOIDS */}
      <section id="gallery" className="relative w-full h-auto md:h-screen md:snap-start flex items-center justify-center py-20 md:py-0 px-6">
        <div className="max-w-6xl w-full space-y-12">
          <div className="text-center">
            <span className="text-salvore-caramel text-xs tracking-widest uppercase font-mono block">04 // Spatial Aesthetic</span>
            <h2 className="font-serif text-3xl md:text-5xl font-bold text-white">Visual Echoes</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {["https://images.unsplash.com/photo-1554118811-1e0d58224f24?q=80&w=500", "https://images.unsplash.com/photo-1498804103079-a6351b050096?q=80&w=500", "https://images.unsplash.com/photo-1507133750040-4a8f57021571?q=80&w=500", "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?q=80&w=500"].map((src, idx) => (
              <div key={idx} className="glass-panel-luxury h-40 md:h-72 rounded-2xl overflow-hidden relative group border border-white/5">
                <img src={src} className="w-full h-full object-cover grayscale-0 md:grayscale md:group-hover:grayscale-0 transition-all duration-700" alt="Gallery" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-4 flex items-end">
                  <span className="text-[10px] font-mono uppercase tracking-widest text-salvore-gold">Framework 0{idx+1}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* PANEL 5: FAQ & CONTACT - THE FINAL COMPRESSION */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div className="space-y-8">
              <div className="space-y-2">
                ...
                <h2 className="font-serif text-4xl font-bold text-white">Visit Us.</h2>
              </div>
