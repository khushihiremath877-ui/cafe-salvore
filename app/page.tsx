...
import { ArrowRight, Compass, Shield, Award, Clock, MapPin, ChevronRight, Calendar, BookOpen } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <main ref={containerRef} className="min-h-screen xl:h-screen w-full overflow-y-auto xl:overflow-y-scroll xl:snap-y xl:snap-mandatory scroll-smooth no-scrollbar text-salvore-cream selection:bg-salvore-caramel selection:text-black relative z-10 pb-12 xl:pb-0">
      ...
      {/* AMBIENT MOUSE SPOTLIGHT IRIS (DISABLED ON TOUCH TO PREVENT JANK) */}
      <div 
      ...
      {/* PANEL 1: HERO */}
      <section id="home" className="min-h-[85vh] xl:h-screen w-full xl:snap-start flex items-center justify-center relative px-4 sm:px-6 overflow-hidden pt-20 xl:pt-0">
        ...
        <div className="relative z-10 flex flex-col items-center max-w-5xl space-y-6 sm:space-y-8 text-center px-2">
          ...
          <div className="pt-2 sm:pt-4">
            <a href="#about" className="group relative glass-panel-luxury text-salvore-cream px-8 sm:px-12 py-4 sm:py-5 rounded-full inline-flex items-center gap-3 sm:gap-4 text-[10px] sm:text-xs tracking-[0.3em] uppercase hover:bg-salvore-caramel hover:text-black hover:border-transparent transition-all duration-700 font-medium cursor-pointer">
              Explore The Estate
      ...
      {/* PANEL 2: ABOUT */}
      <section id="about" className="min-h-screen xl:h-screen w-full xl:snap-start flex items-center justify-center relative px-4 sm:px-6 md:px-20 overflow-hidden py-16 xl:py-0">
        <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 items-center relative z-10">
          <div className="lg:col-span-5 space-y-4 sm:space-y-6">
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-4 border-t border-white/5 pt-4 sm:pt-6 font-mono">
              <div>
                ...
                <div className="text-[8px] sm:text-[10px] text-white/40 uppercase tracking-wider mt-1">Decibels</div>
              </div>
              <div>
                <div className="text-lg sm:text-2xl font-bold text-salvore-gold">100%</div>
                <div className="text-[8px] sm:text-[10px] text-white/40 uppercase tracking-wider mt-1">Micro-Lot</div>
              </div>
              <div>
                <div className="text-lg sm:text-2xl font-bold text-salvore-gold">0.02s</div>
                <div className="text-[8px] sm:text-[10px] text-white/40 uppercase tracking-wider mt-1">Variance</div>
              </div>
            </div>
          </div>

          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 w-full">
            <motion.div whileHover={{ scale: 1.02 }} className="glass-panel-luxury p-6 sm:p-8 rounded-3xl space-y-4 sm:space-y-6 group border border-white/5 transition-all duration-500 hover:border-salvore-caramel/30 cursor-pointer relative overflow-hidden h-[180px] sm:h-[220px]">
              ...
              <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:bg-salvore-caramel group-hover:text-black transition-all duration-500 relative z-10">
                <Compass className="w-4 h-4 sm:w-5 sm:h-5 text-salvore-caramel group-hover:text-black" />
              </div>
              <div className="space-y-1 sm:space-y-2 relative z-10">
                <h3 className="font-serif text-lg sm:text-xl font-bold text-white">Controlled Extraction</h3>
                <p className="text-[11px] sm:text-xs text-white/40 leading-relaxed">Atmospheric bars maintained to preserve moisture curves perfectly.</p>
              </div>
            </motion.div>

            <motion.div whileHover={{ scale: 1.02 }} className="glass-panel-luxury p-6 sm:p-8 rounded-3xl space-y-4 sm:space-y-6 group border border-white/5 transition-all duration-500 hover:border-salvore-caramel/30 cursor-pointer sm:mt-0 xl:mt-10 relative overflow-hidden h-[180px] sm:h-[220px]">
              <div className="absolute inset-0 z-0 opacity-10 group-hover:opacity-20 transition-opacity duration-500">
              ...
              <div className="space-y-1 sm:space-y-2 relative z-10">
                <h3 className="font-serif text-lg sm:text-xl font-bold text-white">Private Alcovian Framing</h3>
                <p className="text-[11px] sm:text-xs text-white/40 leading-relaxed">Seating decks arranged with high acoustic boundaries safeguarding privacy.</p>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      {/* PANEL 3: MENU GRID */}
      <section id="menu" className="min-h-screen xl:h-screen w-full xl:snap-start flex items-center justify-center relative px-4 sm:px-6 md:px-12 overflow-hidden py-16 xl:py-0">
        <div className="max-w-6xl w-full space-y-6 sm:space-y-8 relative z-10">
          <div className="text-center space-y-1 sm:space-y-2">
            ...
            <h2 className="font-serif text-3xl sm:text-5xl font-bold text-white">The Culinary Collection</h2>
          </div>
          
          <div className="flex justify-center gap-1 sm:gap-2 max-w-xs sm:max-w-sm mx-auto glass-panel-luxury p-1 rounded-full border border-white/5">
            {categories.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)} className={`text-[9px] sm:text-[11px] tracking-widest uppercase px-4 sm:px-6 py-2 sm:py-2.5 rounded-full cursor-pointer font-medium transition-all duration-500 flex-1 text-center ${activeCategory === cat ? "bg-gradient-to-r from-salvore-caramel to-salvore-gold text-black shadow-lg" : "text-white/60 hover:text-white"}`}>{cat}</button>
            ))}
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            <AnimatePresence mode="wait">
              <motion.div key={activeCategory} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 col-span-2">
                {menuData[activeCategory].map((item) => (
                  ...
                  >
                    <div className="w-full sm:w-[35%] h-[120px] sm:h-full relative overflow-hidden bg-black shrink-0">
                      <img 
                        src={item.image} 
                        alt={item.name} 
                    ...
                    <div className="p-4 sm:p-5 md:p-6 flex flex-col justify-between flex-1 space-y-2 bg-white/[0.005]">
      ...
      <section id="signature" className="min-h-screen xl:h-screen w-full xl:snap-start flex items-center justify-center relative px-4 sm:px-6 md:px-12 overflow-hidden py-16 xl:py-0">
        <div className="max-w-6xl w-full space-y-8 sm:space-y-12 relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/5 pb-4 sm:pb-6">
            <div className="space-y-1 sm:space-y-2">
              ...
              <h2 className="font-serif text-3xl sm:text-5xl md:text-6xl font-bold text-white">Liquid Artistry</h2>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
            {menuData.Coffee.map((drink, i) => (
              <motion.div key={i} whileHover={{ y: -4 }} className="glass-panel-luxury p-6 sm:p-8 rounded-3xl flex flex-col justify-between h-[180px] sm:h-[220px] relative overflow-hidden group hover:border-salvore-caramel/50 transition-all duration-500 cursor-pointer">
                <div className="space-y-2 sm:space-y-3">
                  <div className="flex justify-between items-baseline">
                    ...
                    <span className="text-gradient-gold-premium font-mono text-sm sm:text-lg font-bold">{drink.price}</span>
                  </div>
                  <h3 className="font-serif text-xl sm:text-2xl font-bold text-white group-hover:text-salvore-gold transition-colors">{drink.name}</h3>
                  <p className="text-white/50 text-[11px] sm:text-xs leading-relaxed line-clamp-3">{drink.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section id="gallery" className="min-h-screen xl:h-screen w-full xl:snap-start flex items-center justify-center relative px-4 sm:px-6 md:px-12 overflow-hidden py-16 xl:py-0">
        <div className="max-w-6xl w-full space-y-6 sm:space-y-8 relative z-10">
          <div className="text-center space-y-1">
            ...
            <h2 className="font-serif text-3xl sm:text-5xl font-bold text-white">Visual Echoes</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
            {[
              ...
              <motion.div key={idx} whileHover={{ scale: 1.02 }} className="glass-panel-luxury h-[160px] sm:h-[260px] rounded-2xl overflow-hidden relative group border border-white/5 cursor-pointer">
                <img src={src} alt="Salvore Spatial Asset" className="w-full h-full object-cover grayscale brightness-70 group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent xl:opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-3 sm:p-4">
      ...
      <section id="events" className="min-h-screen xl:h-screen w-full xl:snap-start flex items-center justify-center relative px-4 sm:px-6 md:px-12 overflow-hidden py-16 xl:py-0">
        <div className="max-w-5xl w-full space-y-6 sm:space-y-8 relative z-10">
          <div className="text-center space-y-1">
            ...
            <h2 className="font-serif text-3xl sm:text-5xl font-bold text-white">Curated Gatherings</h2>
          </div>
          <div className="space-y-4">
            {communityEvents.map((ev, idx) => (
              <div key={idx} className="glass-panel-luxury p-5 sm:p-6 md:p-8 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 group border border-white/5 hover:border-salvore-caramel/30 transition-colors">
                <div className="space-y-1.5 sm:space-y-2">
                  <div className="flex items-center gap-2 text-[9px] sm:text-[10px] font-mono text-salvore-caramel">
                    <Calendar className="w-3 h-3 shrink-0" />
                    <span>{ev.date} — {ev.time}</span>
                  </div>
                  <h3 className="font-serif text-lg sm:text-xl font-bold text-white group-hover:text-salvore-gold transition-colors">{ev.title}</h3>
                  <p className="text-white/50 text-xs max-w-xl leading-relaxed">{ev.desc}</p>
                </div>
                <button className="w-full sm:w-auto text-center px-5 sm:px-6 py-2.5 sm:py-3 border border-white/10 rounded-full text-[10px] sm:text-[11px] font-mono uppercase tracking-wider text-white hover:bg-salvore-cream hover:text-black transition-all duration-300 shrink-0 cursor-pointer">Reserve Pass</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* PANEL 7: THE LEDGER */}
      <section id="blog" className="min-h-screen xl:h-screen w-full xl:snap-start flex items-center justify-center relative px-4 sm:px-6 md:px-12 overflow-hidden py-16 xl:py-0">
        <div className="max-w-4xl w-full space-y-8 sm:space-y-10 relative z-10">
          <div className="text-center space-y-1">
            ...
            <h2 className="font-serif text-3xl sm:text-5xl font-bold text-white">The Salvore Ledger</h2>
          </div>
          <div className="space-y-6 sm:space-y-8">
            {articles.map((art, idx) => (
              <div key={idx} className="border-b border-white/5 pb-4 sm:pb-6 space-y-2 group cursor-pointer">
                <div className="flex items-center gap-2 font-mono text-[9px] sm:text-[10px] text-salvore-caramel">
                  <BookOpen className="w-3 h-3 shrink-0" />
                  <span>{art.date}</span>
                </div>
                <h3 className="font-serif text-lg sm:text-xl font-bold text-white group-hover:text-salvore-gold transition-colors duration-300">{art.title}</h3>
                <p className="text-white/50 text-xs max-w-2xl leading-relaxed line-clamp-2 sm:line-clamp-none">{art.excerpt}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="faq" className="min-h-screen xl:h-screen w-full xl:snap-start flex items-center justify-center relative px-4 sm:px-6 md:px-12 overflow-hidden py-16 xl:py-0">
        <div className="max-w-4xl w-full grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-12 items-start relative z-10">
          <div className="lg:col-span-5 space-y-2 sm:space-y-4">
            ...
            <h2 className="font-serif text-3xl sm:text-5xl font-bold text-white">Sought <br /><span className="text-gradient-gold-premium italic font-light">Answers.</span></h2>
          </div>
          <div className="lg:col-span-7 space-y-2.5 sm:space-y-3 w-full">
            {faqs.map((item, idx) => (
              <div key={idx} className="glass-panel-luxury rounded-2xl overflow-hidden border border-white/5 transition-all duration-300">
                <button onClick={() => setOpenFaqIdx(openFaqIdx === idx ? null : idx)} className="w-full text-left p-4 sm:p-6 flex justify-between items-center text-white font-serif text-sm sm:text-base font-bold cursor-pointer hover:text-salvore-gold transition-colors gap-4">
                  <span>{item.q}</span>
                ...
                <AnimatePresence>
                  {openFaqIdx === idx && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-white/5 bg-white/[0.01]">
                      <p className="p-4 sm:p-6 text-xs text-white/50 leading-relaxed font-light">{item.a}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
      ...
      {/* PANEL 9: COORDINATES */}
      <section id="contact" className="min-h-screen xl:h-screen w-full xl:snap-start flex items-center justify-center relative px-4 text-center overflow-hidden py-16 xl:py-0">
        <div className="max-w-2xl space-y-8 sm:space-y-10 relative z-10 w-full px-2">
          <div className="space-y-1 sm:space-y-2">
            ...
            <h2 className="font-serif text-4xl sm:text-6xl font-bold text-white">Visit The Sanctuary</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left w-full">
            <div className="glass-panel-luxury p-5 sm:p-6 rounded-2xl space-y-2 sm:space-y-3 border border-white/5">
              <MapPin className="w-4 h-4 text-salvore-caramel" />
              <h4 className="text-xs uppercase tracking-wider font-semibold text-white">Location Index</h4>
              <p className="text-xs text-white/50 font-light">102 Luxury Arcade, Lavelle Road, Bangalore, India</p>
            </div>
            <div className="glass-panel-luxury p-5 sm:p-6 rounded-2xl space-y-2 sm:space-y-3 border border-white/5">
              <Clock className="w-4 h-4 text-salvore-caramel" />
              <h4 className="text-xs uppercase tracking-wider font-semibold text-white">Service Matrix</h4>
              <p className="text-xs text-white/50 font-light">Monday — Sunday // 07:00 AM to 11:00 PM</p>
            </div>
          </div>
          <div className="pt-2 sm:pt-4">
            <Link href="/reservations" className="inline-block w-full sm:w-auto text-center text-[10px] sm:text-xs uppercase tracking-[0.3em] font-semibold bg-gradient-to-r from-salvore-caramel to-salvore-gold text-black px-8 sm:px-12 py-4 sm:py-5 rounded-full shadow-2xl hover:opacity-90 active:scale-95 transition-all duration-300">Access Booking Portal</Link>
          </div>
        </div>
      </section>
